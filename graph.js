...
var networkOutputBinding = new Shiny.OutputBinding();
  $.extend(networkOutputBinding, {
    ...
    renderValue: function(el, data) {
		...
		//here logic for drawing cloud based on data argument
		//for each word in cloud proper on click event
		//given in genericWordClick should be set
		
		//for now placeholder, logging words and counts from response:
		posx1 = 400
		posx2 = 500
		posy = 200
		//console.log(data)
  ...
  Shiny.outputBindings.register(networkOutputBinding, 'abc.outputbinding');

//LOGIC

var currentName = ""; //current character name
var currentOrderID = 0; //how many words are clicked
var currentContext = []; //history of previous clicked words

 //logic for name switch
 function newCloudClick (){
	//reset context
	currentContext = []
	currentOrderID = 1;
	//set new name
	currentName = document.getElementById('inputText').value;
	var msg = {name:currentName,orderID:currentOrderID, context:currentContext};
    //send new compute request to shiny
    Shiny.onInputChange("req_data", msg);
 }
 
 //generic function sending request for new cloud, given context
 function genericWordClick (word){
    return (function(){
		//curentName stays
		
		//add to context
		currentOrderID++;
		currentContext.push(word)
		
		var msg = {name:currentName,orderID:currentOrderID, context:currentContext};
		//send new compute request to shiny
		Shiny.onInputChange("req_data", msg);
		//update contextLabel - current phrase on screen
		document.getElementById('contextLabel').value = "Current phrase: "+currentContext.join(" ");
	});
 }
 
 //logic for going back one step
 function goBackClick (evt){
...
//static events
$('#newCloud').click(newCloudClick);
$('#goBack').click(goBackClick);
